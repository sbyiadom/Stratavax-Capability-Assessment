          ...
          if (supabaseSession) {
            ...
            if (userRole === "supervisor" || userRole === "admin") {
              setCurrentSupervisor({
                ...
                name:
                  (supabaseSession.user.user_metadata && supabaseSession.user.user_metadata.full_name) ||
            ...
            candidatesData.map(async function (candidate) {
              ...
              if (assessmentIds.length > 0) {
                assessmentsResponse = await supabase
                  .from("assessments")
                  .select(
                    "id, title, description, assessment_type_id, assessment_types(id, code, name, icon, gradient_start, gradient_end)"
                  )
                  .in("id", assessmentIds);
      ...
      <div style={styles.pageShell}>
          ...
          <div>
            ...
            <p style={styles.heroSubtitle}>
              {currentSupervisor.role === "admin"
                ? "Admin view across all candidates and assessments"
                : "Assigned candidates and assessment progress"}
            </p>
        ...
        <div style={styles.bulkCard}>
          ...
          <p style={styles.bulkText}>
            Select one assessment, load candidates, choose the candidates to reset, run Dry Run, then Bulk Reset Selected.
            Only the selected assessment will be reset.
          </p>
          <div style={styles.bulkGrid}>
            ...
            <button
              ...
              onClick={loadAssessmentResetCandidates}
              disabled={action.loadLoading || !resetAssessmentId}
            >
          ...
          {resetCandidates.length > 0 && (
            <div style={styles.resetCandidatePanel}>
              <div style={styles.resetCandidateHeader}>
                ...
                <button type="button" style={styles.lightButton} onClick={selectAllLoadedCandidates}>
                  Select All
                </button>
                <button type="button" style={styles.lightButton} onClick={clearSelectedCandidates}>
                  Clear
                </button>
                <button
                  type="button"
                  ...
                  onClick={function () {
                    runAssessmentBulkReset(true);
                  }}
                  disabled={action.dryRunLoading || action.loading || selectedCount === 0}
                >
                  ...
                  return (
                    <label key={candidate.userId} style={selected ? styles.resetCandidateSelected : styles.resetCandidateRow}>
                      <input
                        ...
                        checked={selected}
                        onChange={function () {
                          toggleCandidateSelection(candidate.userId);
                        }}
                      />
                      ...
                      <Pill color="#175cd3" bg="#eff8ff">
                        {candidate.resetCategory}
                      </Pill>
          ...
          {action.result && (
            <div style={styles.bulkSummary}>
              <strong>Summary:</strong> {safeNumber(action.result.successful, 0)} successful, {safeNumber(action.result.failed, 0)} failed,
              {safeNumber(action.result.totalDeduped || action.result.count, 0)} total.
            </div>
          )}
        </div>
          ...
          <div style={styles.contentHeader}>
            <div>
              ...
              <p style={styles.contentSubtitle}>
                {filteredCandidates.length} of {candidates.length} candidate(s) shown
              </p>
            </div>
            <Pill color="#175cd3" bg="#eff8ff">
              Assessment-specific reset enabled
            </Pill>
          </div>
              ...
              {filteredCandidates.map(function (candidate) {
                ...
                var latestPercentage =
                  latestAssessment && latestAssessment.result
                    ? calculatePercentage(latestAssessment.result.score, latestAssessment.result.max_score, latestAssessment.result.percentage)
                    : 0;
                    ...
                    <div style={styles.candidateMainRow}>
                      ...
                      <div style={styles.scoreBlock}>
                        <div style={styles.scoreTopLine}>
                          ...
                          <Pill color={classification.color} bg={classification.bg}>
                            {classification.label}
                          </Pill>
                        </div>
                        <ProgressBar value={latestPercentage} color={classification.color} />
                        <p style={styles.lastActive}>
                          Last completed: {latestAssessment && latestAssessment.result ? formatDate(latestAssessment.result.completed_at) : "Never"}
                        </p>
                      </div>
                      <div style={styles.summaryPills}>
                        <Pill color="#027a48" bg="#ecfdf3">
                          ✓ {candidate.completedAssessments} completed
                        </Pill>
                        <Pill color="#175cd3" bg="#eff8ff">
                          ↗ {candidate.readyAssessments} ready
                        </Pill>
                        <Pill color="#c2410c" bg="#fff7ed">
                      ...
                      <div style={styles.assessmentPanel}>
                        <div style={styles.assessmentGrid}>
                          {candidate.assessments.map(function (assessment) {
                            ...
                            var percentage = assessment.result
                              ? calculatePercentage(assessment.result.score, assessment.result.max_score, assessment.result.percentage)
                              : 0;
                            ...
                            return (
                              <div key={assessment.id} style={styles.assessmentCard}>
                                  ...
                                  <div
                                    style={{
                                      ...
                                      background:
                                        ...
                                        assessment.type_gradient_start +
                                        " 0%, " +
                                        assessment.type_gradient_end +
                                        " 100%)"
                                    }}
                                  >
                                ...
                                <div style={styles.assessmentPills}>
                                  ...
                                  {assessment.result && (
                                    <Pill color={assessmentClassification.color} bg={assessmentClassification.bg}>
                                      {percentage}%
                                    </Pill>
                                  )}
                                </div>
                                ...
                                {assessment.result ? (
                                  <React.Fragment>
                                    ...
                                    <Link
                                      href={`/supervisor/${candidate.id}?assessment=${assessment.assessment_id}`}
                                  ...
                                  <p style={styles.waitingText}>
                                    {assessment.status === "unblocked"
                                      ? "Candidate can take this assessment."
                                      : "Assessment is currently blocked or scheduled."}
                                  </p>
                                )}
                              </div>
                            );
