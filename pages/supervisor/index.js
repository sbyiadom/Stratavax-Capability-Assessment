...
export default function AssignAssessment() {
  ...
  // Load candidate and assessments
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        const { data: candidateData, error: candidateError } = await supabase
          ...
          .select('*')
          .eq('id', candidateId)
          .eq('supervisor_id', currentSupervisor.id) // Ensure this candidate belongs to this supervisor
          .single();
        ...
        // Load already assigned assessments for this candidate
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Find assessments to add
      const toAdd = selectedIds.filter(id => !currentIds.includes(id));

      // Find assessments to remove (optional - if you want to allow unassigning)
      ...
      // Remove unselected assignments (optional)
