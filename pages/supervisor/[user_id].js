...
function getClassification(report) {
  var r = safeObject(report);
  return r.classification || r.overallClassification || r.performanceBand || r.label || "Not classified";
}

function getRiskLevel(report) {
  var r = safeObject(report);
  return r.riskLevel || r.risk_level || "Not available";
}
          ...
          <section style={styles.errorCard}>
            ...
            <div style={styles.debugBox}>
              ...
              <p style={styles.debugText}>
                API route expected: <strong>/api/supervisor/report?user_id={safeText(userId, "")}{assessmentId ? "&assessment_id=" + assessmentId : ""}</strong>
              </p>
          ...
          <React.Fragment>
            <section style={styles.grid}>
              <div style={styles.card}>
                ...
                <p style={styles.bodyText}>
                  {safeText(
                    cleanReport.executiveSummary || cleanReport.summary || cleanReport.overallAssessment || cleanReport.interpretation,
              ...
              <div style={styles.card}>
                ...
                <p style={styles.bodyText}>
                  {safeText(
                    cleanReport.supervisorImplication || cleanReport.supervisor_implication || cleanReport.recommendationSummary,
              ...
              <section style={styles.card}>
                ...
                <div style={styles.metricGrid}>
                  <div style={styles.metricBox}>
                    ...
                    <p style={styles.metricValue}>{safeNumber(behavioralInsights.averageTimePerQuestion, 0)}s</p>
                  </div>
                  <div style={styles.metricBox}>
                    <p style={styles.metricLabel}>Average Answer Changes</p>
                    <p style={styles.metricValue}>{safeNumber(behavioralInsights.averageChangesPerQuestion, 0)}</p>
                  </div>
                  <div style={styles.metricBox}>
                    <p style={styles.metricLabel}>Total Changes</p>
                    <p style={styles.metricValue}>{safeNumber(behavioralInsights.totalChanges, 0)}</p>
                  </div>
                </div>
            ...
            <section style={styles.card}>
              ...
              {categoryScores.length === 0 ? (
                <p style={styles.mutedText}>No category or competency scores were found in the generated report.</p>
              ) : (
                        ...
                        return (
                          <tr key={index}>
                            <td style={styles.td}>
                              ...
                              {row.questionCount && <div style={styles.cellMeta}>{row.questionCount} question(s)</div>}
                            </td>
                            <td style={styles.td}><strong>{formatPercentage(rowPercentage)}</strong></td>
                            <td style={styles.td}>{safeText(row.classification || row.label || row.scoreLevel, "Not classified")}</td>
                            <td style={styles.td}>{safeText(row.riskLevel || row.risk_level, "N/A")}</td>
                            <td style={styles.td}>
                              <p style={styles.tableNarrative}>{rowNarrative}</p>
                              {rowAction && <p style={styles.tableAction}><strong>Action:</strong> {rowAction}</p>}
                            </td>
                          </tr>
                        );
                      })}
                      ...
                      return (
                        <li key={index} style={styles.listItem}>
                          <div>
                            ...
                            <p style={styles.listDescription}>{getRowNarrative(row)}</p>
                          </div>
                          <span style={styles.listMeta}>{formatPercentage(getRowPercentage(row))}</span>
                        </li>
                      );
                    })}
                  </ul>
                    ...
                    {safeArray(developmentAreas).map(function (item, index) {
                      ...
                      return (
                        <li key={index} style={styles.listItem}>
                          ...
                          <span style={styles.listMetaWarning}>{formatPercentage(getRowPercentage(row))}</span>
                        </li>
                      );
                    })}
                  </ul>
                    ...
                    return (
                      <article key={index} style={styles.recommendationCard}>
                        <div style={styles.recommendationHeader}>
                          ...
                          <span style={priorityStyle}>{safeText(row.priority, "Medium")}</span>
                        </div>
                        <p style={styles.bodyText}>{safeText(row.recommendation || row.description, "No recommendation text available.")}</p>
                        {row.action && <p style={styles.actionText}><strong>Action:</strong> {row.action}</p>}
                        {row.impact && <p style={styles.mutedText}><strong>Impact:</strong> {row.impact}</p>}
                      </article>
                    );
                  })}
                </div>
