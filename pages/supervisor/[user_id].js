      ...
      <div style={styles.container}>
        <header style={styles.header}>
          <div>
            ...
            <p style={styles.subtitle}>Assessment: {safeText(assessmentName, "Assessment")}</p>
            <p style={styles.subtitle}>Candidate ID: {safeText(userId, "Not available")}</p>
            <p style={styles.subtitle}>Assessment ID: {safeText(assessmentId, "Not available")}</p>
          </div>
        ...
        {!loading && !errorMessage && (
          <React.Fragment>
              ...
              <section style={styles.card}>
                ...
                {showBehavioralMetrics && (
                  <div style={styles.metricGrid}>
                    <div style={styles.metricBox}><p style={styles.metricLabel}>Average Time / Question</p><p style={styles.metricValue}>{safeNumber(behavioralInsights.averageTimePerQuestion, 0)}s</p></div>
                    <div style={styles.metricBox}><p style={styles.metricLabel}>Average Answer Changes</p><p style={styles.metricValue}>{safeNumber(behavioralInsights.averageChangesPerQuestion, 0)}</p></div>
                    <div style={styles.metricBox}><p style={styles.metricLabel}>Total Changes</p><p style={styles.metricValue}>{safeNumber(behavioralInsights.totalChanges, 0)}</p></div>
                  </div>
                )}
            ...
            <section style={styles.card}>
              <div style={styles.sectionHeader}><h2 style={styles.sectionTitle}>Category / Competency Scores</h2><span style={styles.badge}>{categoryScores.length}</span></div>
              {categoryScores.length === 0 ? <p style={styles.mutedText}>No category or competency scores were found in the generated report.</p> : (
                <div style={styles.tableWrapper}>
                  <table style={styles.table}>
                    <thead><tr><th style={styles.th}>Area</th><th style={styles.th}>Score</th><th style={styles.th}>Classification</th><th style={styles.th}>Risk</th><th style={styles.th}>Narrative Meaning</th></tr></thead>
                    <tbody>{categoryScores.map(function (item, index) {
                      ...
                      return <tr key={index}><td style={styles.td}><strong>{rowTitle}</strong>{row.questionCount && <div style={styles.cellMeta}>{row.questionCount} question(s)</div>}</td><td style={styles.td}><strong>{formatPercentage(rowPercentage)}</strong></td><td style={styles.td}>{safeText(row.classification || row.label || row.scoreLevel, "Not classified")}</td><td style={styles.td}>{safeText(row.riskLevel || row.risk_level, "N/A")}</td><td style={styles.td}><p style={styles.tableNarrative}>{rowNarrative}</p>{rowAction && <p style={styles.tableAction}><strong>Action:</strong> {rowAction}</p>}</td></tr>;
                    })}</tbody>
                  </table>
                </div>
              )}
            ...
            <section style={styles.grid}>
              <div style={styles.card}>
                <div style={styles.sectionHeader}><h2 style={styles.sectionTitle}>Top Strengths</h2><span style={styles.badge}>{safeArray(strengths).length}</span></div>
                {safeArray(strengths).length === 0 ? <p style={styles.mutedText}>No strengths available.</p> : <ul style={styles.list}>{safeArray(strengths).map(function (item, index) { var row = safeObject(item); return <li key={index} style={styles.listItem}><div><strong>{getRowTitle(row)}</strong><p style={styles.listDescription}>{getRowNarrative(row)}</p></div><span style={styles.listMeta}>{formatPercentage(getRowPercentage(row))}</span></li>; })}</ul>}
              </div>
              <div style={styles.card}>
                <div style={styles.sectionHeader}><h2 style={styles.sectionTitle}>Development Areas</h2><span style={styles.badge}>{safeArray(developmentAreas).length}</span></div>
                {safeArray(developmentAreas).length === 0 ? <p style={styles.mutedText}>No development areas available.</p> : <ul style={styles.list}>{safeArray(developmentAreas).map(function (item, index) { var row = safeObject(item); return <li key={index} style={styles.listItem}><div><strong>{getRowTitle(row)}</strong><p style={styles.listDescription}>{getRowNarrative(row)}</p></div><span style={styles.listMetaWarning}>{formatPercentage(getRowPercentage(row))}</span></li>; })}</ul>}
              </div>
            </section>

            <section style={styles.card}>
              <div style={styles.sectionHeader}><h2 style={styles.sectionTitle}>Supervisor Follow-up Questions</h2><span style={styles.badge}>{followUpQuestions.length}</span></div>
              {followUpQuestions.length === 0 ? <p style={styles.mutedText}>No follow-up questions have been generated yet.</p> : <div style={styles.questionList}>{followUpQuestions.map(function (item, index) { var row = safeObject(item); return <article key={index} style={styles.questionCard}><div style={styles.recommendationHeader}><h3 style={styles.recommendationTitle}>{safeText(row.category, "Follow-up Area")}</h3><span style={getPriorityStyle(row.priority)}>{safeText(row.priority, "Medium")}</span></div><p style={styles.bodyText}>{safeText(row.question, "No question text available.")}</p></article>; })}</div>}
            </section>

            <section style={styles.card}>
              <div style={styles.sectionHeader}><h2 style={styles.sectionTitle}>Recommendations</h2><span style={styles.badge}>{recommendations.length}</span></div>
              {recommendations.length === 0 ? <p style={styles.mutedText}>No recommendations have been generated yet.</p> : <div style={styles.recommendationList}>{recommendations.map(function (item, index) { var row = safeObject(item); var priorityStyle = getPriorityStyle(row.priority); return <article key={index} style={styles.recommendationCard}><div style={styles.recommendationHeader}><h3 style={styles.recommendationTitle}>{safeText(row.competency || row.category || row.title, "Recommendation")}</h3><span style={priorityStyle}>{safeText(row.priority, "Medium")}</span></div><p style={styles.bodyText}>{safeText(row.recommendation || row.description, "No recommendation text available.")}</p>{row.action && <p style={styles.actionText}><strong>Action:</strong> {safeText(row.action, "")}</p>}{row.impact && <p style={styles.mutedText}><strong>Impact:</strong> {safeText(row.impact, "")}</p>}{row.followUpQuestion && <p style={styles.mutedText}><strong>Follow-up:</strong> {safeText(row.followUpQuestion, "")}</p>}</article>; })}</div>}
            </section>
          </React.Fragment>
        )}
      </div>
...
var styles = {
  page: { minHeight: "100vh", background: "#f4f6f8", padding: "32px 16px", color: "#172033", fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Arial, sans-serif' },
  container: { maxWidth: "1180px", margin: "0 auto" },
  header: { display: "flex", justifyContent: "space-between", gap: "24px", alignItems: "stretch", marginBottom: "24px", flexWrap: "wrap" },
  eyebrow: { margin: "0 0 6px", color: "#667085", fontSize: "14px", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.05em" },
  title: { margin: 0, fontSize: "34px", lineHeight: 1.2, color: "#101828" },
  subtitle: { margin: "8px 0 0", color: "#667085", fontSize: "14px" },
  scoreCard: { minWidth: "230px", background: "#ffffff", borderRadius: "18px", padding: "22px", boxShadow: "0 10px 30px rgba(16, 24, 40, 0.08)", border: "1px solid #eaecf0" },
  scoreLabel: { margin: 0, color: "#667085", fontSize: "13px", fontWeight: 700, textTransform: "uppercase" },
  ...
  errorCard: { background: "#fff5f5", borderRadius: "18px", padding: "22px", border: "1px solid #fecaca", marginBottom: "18px" },
  sectionHeader: { display: "flex", justifyContent: "space-between", alignItems: "center", gap: "12px", marginBottom: "14px" },
  sectionTitle: { margin: 0, fontSize: "20px", color: "#101828" },
  badge: { display: "inline-flex", alignItems: "center", justifyContent: "center", minWidth: "34px", height: "28px", padding: "0 10px", borderRadius: "999px", background: "#eef4ff", color: "#3538cd", fontWeight: 700, fontSize: "13px" },
  badgeWarning: { display: "inline-flex", alignItems: "center", justifyContent: "center", minWidth: "34px", height: "28px", padding: "0 10px", borderRadius: "999px", background: "#fff7ed", color: "#c2410c", fontWeight: 700, fontSize: "13px" },
  bodyText: { margin: 0, color: "#344054", lineHeight: 1.65, fontSize: "15px" },
  mutedText: { margin: "8px 0 0", color: "#667085", lineHeight: 1.6, fontSize: "14px" },
  errorText: { margin: "10px 0", color: "#b42318", fontWeight: 700 },
  tableWrapper: { overflowX: "auto" },
  table: { width: "100%", borderCollapse: "collapse", fontSize: "14px" },
  th: { textAlign: "left", padding: "12px", background: "#f9fafb", color: "#475467", borderBottom: "1px solid #eaecf0", fontWeight: 700, verticalAlign: "top" },
  td: { padding: "12px", borderBottom: "1px solid #eaecf0", color: "#344054", verticalAlign: "top" },
  tableNarrative: { margin: 0, color: "#344054", lineHeight: 1.55 },
  tableAction: { margin: "8px 0 0", color: "#475467", lineHeight: 1.55, fontSize: "13px" },
  cellMeta: { marginTop: "4px", color: "#667085", fontSize: "12px" },
  list: { listStyle: "none", margin: 0, padding: 0 },
  listItem: { display: "flex", justifyContent: "space-between", gap: "12px", padding: "12px 0", borderBottom: "1px solid #eaecf0", color: "#344054" },
  listDescription: { margin: "4px 0 0", color: "#667085", fontSize: "13px", lineHeight: 1.5 },
  listMeta: { fontWeight: 800, color: "#0f766e", whiteSpace: "nowrap" },
  listMetaWarning: { fontWeight: 800, color: "#c2410c", whiteSpace: "nowrap" },
  recommendationList: { display: "grid", gap: "14px" },
  recommendationCard: { padding: "16px", borderRadius: "14px", background: "#f9fafb", border: "1px solid #eaecf0" },
  recommendationHeader: { display: "flex", justifyContent: "space-between", gap: "12px", alignItems: "center", marginBottom: "8px" },
  recommendationTitle: { margin: 0, fontSize: "16px", color: "#101828" },
  priorityHigh: { padding: "5px 10px", borderRadius: "999px", background: "#fff7ed", color: "#c2410c", fontSize: "12px", fontWeight: 800, whiteSpace: "nowrap" },
  priorityCritical: { padding: "5px 10px", borderRadius: "999px", background: "#fef3f2", color: "#b42318", fontSize: "12px", fontWeight: 800, whiteSpace: "nowrap" },
  priorityMedium: { padding: "5px 10px", borderRadius: "999px", background: "#fffaeb", color: "#b54708", fontSize: "12px", fontWeight: 800, whiteSpace: "nowrap" },
  priorityLeverage: { padding: "5px 10px", borderRadius: "999px", background: "#ecfdf3", color: "#027a48", fontSize: "12px", fontWeight: 800, whiteSpace: "nowrap" },
  actionText: { margin: "10px 0 0", color: "#344054", lineHeight: 1.6, fontSize: "14px" },
  metricGrid: { display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: "12px", marginBottom: "16px" },
  metricBox: { padding: "14px", borderRadius: "12px", background: "#f9fafb", border: "1px solid #eaecf0" },
