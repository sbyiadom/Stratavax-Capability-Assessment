...
function CandidateReportComponent() {
  ...
  const loadAssessmentData = async (result, candidateInfo) => {
    try {
      ...
      // Extract behavioral insights from interpretations
      if (result.interpretations?.behavioralInsights) {
        setBehavioralData(result.interpretations.behavioralInsights);
        console.log("✅ Loaded behavioral insights:", result.interpretations.behavioralInsights.work_style);
      } else {
        console.log("⚠️ No behavioral insights found in assessment result");
      }
    ...
    <AppLayout background="/images/preassessmentbg.jpg">
      <div style={styles.container}>
        ...
        <div style={styles.navigation}>
          ...
          <button 
            style={{...styles.navItem, borderBottom: activeSection === 'behavioral' ? '3px solid #0A1929' : '3px solid transparent'}}
        ...
        <div ref={reportRef} style={styles.reportContainer}>
          ...
          {/* SECTION 4: BEHAVIORAL INSIGHTS - NEW SECTION */}
          {behavioralData && (
          ...
          {/* SECTION 7: DEVELOPMENT RECOMMENDATIONS */}
...
// All existing styles remain the same (they are too long to reprint, keep your existing styles object)
const styles = {
  // ... (keep all your existing styles from your current file)
