// pages/api/admin/templates/create.js
// Phase 3 Item 5 — Assessment templates
// Creates one template + its role links via create_template RPC.
...
function validateBody(body) {
  ...
  const {
    name, title_pattern, assessment_type_id, description, instructions,
    default_is_active, expires_in_days, is_active, role_ids
  } = body;
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { data, error } = await serviceClient.rpc('create_template', {
      p_name: name.trim(),
    ...
    if (error) {
      console.error('[Templates Create] rpc error:', error);
      const code = error.code;
      if (code === 'P0002') {
        return res.status(404).json({
          success: false,
          error: error.message || 'Assessment type not found'
        });
      }
      if (code === '22023') {
    ...
    console.error('[Templates Create] API error:', error);
    return res.status(500).json({
      success: false,
