...
function AssessmentContent() {
  ...
  // NEW: Check if time has expired
  useEffect(() => {
    if (remainingSeconds <= 0 && !alreadySubmitted && !isAutoSubmitting) {
  ...
  // Timer effect - auto-submit when time expires
  useEffect(() => {
    if (loading || alreadySubmitted || accessDenied || !session || isAutoSubmitting || isTimeExpired) return;
  ...
  // Check if disabled (time expired or already submitted)
