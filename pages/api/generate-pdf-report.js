...
function cleanText(value, fallback) {
  ...
  return decodeHtmlEntities(normalizeText(value, fallback || ""));
}
...
function isBaselineAssessment(assessment) {
  var type = asObject(assessment && assessment.assessment_type) || null;
  var code = cleanText(getValue(type, "code", ""), "").toLowerCase();
  var assessmentTypeId = toNumber(getValue(assessment, "assessment_type_id", null), 0);

  return isBaselineAssessmentType(assessmentTypeId) || isBaselineAssessmentType(code);
}
...
async function fetchAssessment(supabase, assessmentId) {
  var assessment;
  assessment = await selectSingle(
    supabase,
    "assessments",
    function (query) { return query.eq("id", assessmentId); },
...
function buildCategoryScores(responses, isBaseline) {
  var grouped = {};

  safeArray(responses).forEach(function (response) {
    var category = getResponseCategory(response);
    var subcategory = getResponseSubcategory(response);
    var scored = scoreQuestionResponse(response, Boolean(isBaseline));
    var score = toNumber(scored.score, 0);
    var maxScore = toNumber(scored.maxScore, 0);
...
function buildExecutiveSummary(candidateName, assessmentName, percentage, classification, categoryScores) {
  var lowestAreas = safeArray(categoryScores)
    .filter(function (item) { return toNumber(item.percentage, 0) < 65; })
    .sort(function (a, b) { return toNumber(a.percentage, 0) - toNumber(b.percentage, 0); })
    .slice(0, 3);
  var topAreas = safeArray(categoryScores)
    .filter(function (item) { return toNumber(item.percentage, 0) >= 75; })
    .sort(function (a, b) { return toNumber(b.percentage, 0) - toNumber(a.percentage, 0); })
    .slice(0, 3);
  var strengthText = "";
...
function buildFollowUpQuestions(categoryScores) {
  var selected = safeArray(categoryScores)
    .filter(function (item) { return toNumber(item.percentage, 0) < 75; })
    .sort(function (a, b) { return toNumber(a.percentage, 0) - toNumber(b.percentage, 0); });
...
function buildReport(candidate, assessment, responses, userId, assessmentId, source) {
  ...
  baseline = isBaselineAssessment(assessment);

  safeArray(responses).forEach(function (response) {
    var scored = scoreQuestionResponse(response, baseline);
    totalScore += toNumber(scored.score, 0);
  });

  questions = safeArray(responses)
    .map(function (response) { return getQuestion(response); })
    .filter(function (question) { return question && question.id !== undefined && question.id !== null; });

  maxScore = calculateMaxScore(questions, baseline ? 1 : 0, baseline);
  if (maxScore > 0) percentage = roundNumber((totalScore / maxScore) * 100, 2);

  classification = classifyScore(percentage);
  categoryScores = buildCategoryScores(responses, baseline);
...
function buildCategoryRows(items) {
  var rows = safeArray(items).map(function (item) {
    return '<tr><td><strong>' + escapeHtml(item.name || item.category) + '</strong><br><small>' + escapeHtml(item.questionCount || 0) + ' question(s)</small></td><td><strong>' + escapeHtml(item.totalScore) + ' / ' + escapeHtml(item.maxScore) + '</strong><br><small>' + escapeHtml(item.percentage) + '%</small></td><td>' + escapeHtml(item.classification) + '</td><td>' + escapeHtml(item.riskLevel) + '</td><td>' + escapeHtml(item.narrative) + '<br><small><strong>Action:</strong> ' + escapeHtml(item.action) + '</small></td></tr>';
  }).join('');
