...
/*
  Supervisor Report API - Ampersand Fix
  This full file decodes HTML entities before returning report text.
*/
...
async function fetchResponses(supabase, userId, assessmentId) {
  ...
  if (assessmentId) {
    resultWithAssessment = await selectRows(supabase, "responses", function (query) { return query.eq("user_id", userId).eq("assessment_id", assessmentId); }, selectText);
    if (resultWithAssessment.data.length > 0) return { responses: resultWithAssessment.data, source: "responses.user_id_and_assessment_id_with_joins" };
    resultPlainAssessment = await selectRows(supabase, "responses", function (query) { return query.eq("user_id", userId).eq("assessment_id", assessmentId); }, "*");
    if (resultPlainAssessment.data.length > 0) return { responses: resultPlainAssessment.data, source: "responses.user_id_and_assessment_id_plain" };
  }
  resultUserOnly = await selectRows(supabase, "responses", function (query) { return query.eq("user_id", userId); }, selectText);
...
function buildRecommendations(categoryScores) {
  ...
  strengths.forEach(function (area) { recommendations.push({ priority: "Leverage", competency: area.name, category: area.name, currentScore: area.percentage, gap: 0, recommendation: area.name + " is a relative strength. The supervisor can use this area to support confidence, contribution, and role performance.", action: "Assign role-relevant tasks that allow the candidate to apply " + area.name + " in practical work situations.", impact: "Using this strength can improve candidate contribution and development momentum.", isStrength: true }); });
...
function buildReport(candidate, assessment, responses, userId, assessmentId, source) {
  ...
  return { candidateName: candidateName, candidate_name: candidateName, assessmentName: assessmentName, assessment_name: assessmentName, userId: userId, user_id: userId, assessmentId: assessmentId || null, assessment_id: assessmentId || null, totalScore: roundNumber(totalScore, 2), total_score: roundNumber(totalScore, 2), maxScore: roundNumber(maxScore, 2), max_score: roundNumber(maxScore, 2), percentage: percentage, overallPercentage: percentage, overall_score: percentage, classification: classification, overallClassification: classification, riskLevel: getRiskLevel(percentage), summary: executiveSummary, executiveSummary: executiveSummary, overallAssessment: executiveSummary, interpretation: executiveSummary, supervisorImplication: getSupervisorImplication(percentage), roleReadiness: roleReadiness, role_readiness: roleReadiness, behavioralInsights: behavioralInsights, behavioralSummary: behavioralInsights, categoryScores: categoryScores, category_scores: categoryScores, competencyScores: categoryScores, competency_scores: categoryScores, strengths: strengths, topStrengths: strengths, developmentAreas: developmentAreas, topDevelopmentNeeds: developmentAreas, recommendations: recommendations, followUpQuestions: followUpQuestions, follow_up_questions: followUpQuestions, responseCount: safeArray(responses).length, response_count: safeArray(responses).length, dataSource: source };
}
...
export default async function handler(req, res) {
  ...
  if (!supabaseUrl || !supabaseKey) return res.status(500).json({ success: false, error: "Supabase environment variables are missing. Set NEXT_PUBLIC_SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY." });
