// pages/candidate/dashboard.js - PROFESSIONAL VERSION
...
export default function CandidateDashboard() {
  ...
  const getStatusColor = (status) => {
    switch(status) {
      case 'unblocked': return { bg: '#dcfce7', color: '#166534', label: 'Ready to Start', icon: '🚀' };
      case 'in_progress': return { bg: '#fef3c7', color: '#92400e', label: 'In Progress', icon: '⏳' };
      case 'completed': return { bg: '#dbeafe', color: '#1e40af', label: 'Completed', icon: '✅' };
      default: return { bg: '#f1f5f9', color: '#64748b', label: 'Blocked', icon: '🔒' };
    }
  };
                  ...
                  return (
                    <div key={assessment.id} style={styles.assessmentCard}>
                      ...
                      <div style={styles.cardMeta}>
                        ...
                        <span style={styles.cardType}>{assessment.typeName}</span>
                      </div>
...
const styles = {
  ...
  sectionCount: { fontSize: "14px", color: "rgba(255,255,255,0.7)", textShadow: "1px 1px 2px rgba(0,0,0,0.3)" },
  
  assessmentGrid: { display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", gap: "20px" },
  assessmentCard: { background: "white", borderRadius: "16px", padding: "24px", boxShadow: "0 4px 20px rgba(0,0,0,0.1)", transition: "transform 0.2s, box-shadow 0.2s", display: "flex", flexDirection: "column" },
  ...
  cardType: { padding: "2px 8px", background: "#f1f5f9", borderRadius: "4px", fontSize: "12px" },
  ...
  infoNote: { padding: "12px 20px", background: "rgba(227,242,253,0.95)", borderRadius: "10px", display: "flex", alignItems: "center", gap: "10px", color: "#1565c0", fontSize: "14px", border: "1px solid #90caf9", marginTop: "20px" },
