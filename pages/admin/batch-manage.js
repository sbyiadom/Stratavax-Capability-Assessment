...
// Phase 7A: fetches candidates + supervisors from a single server-side
// endpoint instead of reading Supabase directly. Prepares for RLS.
...
// ============================================================
function SupervisorAssignmentModal({ candidate, allSupervisors, currentAssignments, onClose, onSave }) {
  ...
  useEffect(() => {
    const current = Array.isArray(currentAssignments) ? currentAssignments : [];
    setSelectedSupervisors(current);
    setAvailableSupervisors(
      (allSupervisors || []).filter((s) => !current.includes(s.id))
    );
  }, [candidate, allSupervisors, currentAssignments]);
    ...
    <div style={modalStyles.overlay} onClick={onClose}>
      <div style={modalStyles.modal} onClick={e => e.stopPropagation()}>
              ...
              {selectedSupervisors.map(id => {
                ...
                return (
                  <div key={id} style={modalStyles.chip}>
                    <span>{name}</span>
                    <button onClick={() => removeSupervisor(id)} style={modalStyles.removeChip}>×</button>
                  </div>
                );
              })}
            </div>
            ...
            {availableSupervisors.length === 0 ? (
              ...
              availableSupervisors.map(supervisor => (
                <div key={supervisor.id} style={modalStyles.availableItem}>
                  <div>
                    <div style={modalStyles.supervisorName}>{supervisor.full_name || 'Unknown'}</div>
                    <div style={modalStyles.supervisorEmail}>{supervisor.email || ''}</div>
                  </div>
                  <button onClick={() => addSupervisor(supervisor.id)} style={modalStyles.addButton}>+ Add</button>
                </div>
              ))
            )}
          </div>
        ...
        <div style={modalStyles.actions}>
          ...
          <button onClick={handleAssign} style={modalStyles.saveButton} disabled={saving}>
...
const modalStyles = {
  ...
  modal: { background: 'white', borderRadius: '16px', padding: '24px', maxWidth: '600px', width: '100%', maxHeight: '80vh', overflow: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,0.3)' },
  ...
  candidateInfo: { background: '#f8fafc', padding: '12px 16px', borderRadius: '8px', marginBottom: '16px', border: '1px solid #e2e8f0' },
  candidateEmail: { display: 'block', fontSize: '13px', color: '#64748b', marginTop: '2px' },
  ...
  chip: { display: 'inline-flex', alignItems: 'center', gap: '8px', background: '#e2e8f0', padding: '4px 12px 4px 16px', borderRadius: '20px', fontSize: '13px', color: '#1a202c' },
  removeChip: { background: 'none', border: 'none', cursor: 'pointer', fontSize: '16px', color: '#991b1b', padding: '0 4px', display: 'flex', alignItems: 'center', justifyContent: 'center' },
  availableList: { display: 'flex', flexDirection: 'column', gap: '8px', maxHeight: '200px', overflow: 'auto', border: '1px solid #e2e8f0', borderRadius: '8px', padding: '8px' },
  availableItem: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 12px', border: '1px solid #f1f5f9', borderRadius: '8px', background: '#fafbfc' },
  supervisorName: { fontSize: '14px', fontWeight: '500', color: '#1a202c' },
  supervisorEmail: { fontSize: '12px', color: '#94a3b8' },
  addButton: { padding: '4px 16px', background: '#1a237e', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer', fontSize: '13px', fontWeight: '500', whiteSpace: 'nowrap' },
  ...
  saveButton: { padding: '8px 24px', background: '#1a237e', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontSize: '14px', fontWeight: '600' },
  emptyText: { color: '#94a3b8', fontSize: '13px', fontStyle: 'italic', margin: '4px 0' },
...
export default function AdminBatchManageRedirect() {
  ...
  async function checkAdminAndLoad() {
    try {
      ...
      await loadData(activeSession.access_token);
    } catch (error) {
  ...
  const loadData = async (token) => {
    try {
      ...
      setSupervisorList(Array.isArray(payload.supervisors) ? payload.supervisors : []);
      setAssignmentsByCandidate(payload.assignments || {});
    } catch (error) {
      console.error('Error loading admin batch data:', error);
      setMessage({ type: 'error', text: getReadableError(error) });
    }
  };
    ...
    <AppLayout background="/images/admin-bg.jpg">
                ...
                <tbody>
                  {candidates.length === 0 ? (
                    ...
                    candidates.map((candidate) => {
                      const primaryName = candidate.supervisor_id
                      ...
                      return (
                        <tr key={candidate.id} style={styles.tr}>
                          <td style={styles.td}>
                            <div style={styles.candidateName}>{candidate.full_name || 'Unknown'}</div>
                          ...
                          <td style={styles.td}>
                            {primaryName ? (
                              <span style={styles.supervisorBadge}>{primaryName}</span>
                            ) : (
      ...
      {showAssignmentModal && selectedCandidate && (
        <SupervisorAssignmentModal
          ...
          currentAssignments={assignmentsByCandidate[selectedCandidate.id] || (selectedCandidate.supervisor_id ? [selectedCandidate.supervisor_id] : [])}
...
const styles = {
  ...
  backButton: { padding: '8px 16px', background: 'transparent', border: '1px solid #e2e8f0', borderRadius: '8px', cursor: 'pointer', fontSize: '14px', color: '#475569', marginBottom: '16px' },
  title: { margin: "0 0 8px", color: "#0a1929", fontSize: "26px", fontWeight: 800 },
  subtitle: { margin: "0 0 24px", color: "#667085", fontSize: "14px", lineHeight: 1.6 },
  supervisorSummary: { display: 'flex', alignItems: 'center', gap: '8px', padding: '10px 16px', background: '#f8fafc', borderRadius: '8px', border: '1px solid #e2e8f0', marginBottom: '16px', flexWrap: 'wrap', fontSize: '14px', color: '#475569' },
  supervisorTag: { padding: '2px 10px', background: '#e2e8f0', borderRadius: '12px', fontSize: '12px', color: '#1a202c' },
  loadingBlock: { display: "flex", alignItems: "center", justifyContent: "center", gap: "12px", marginBottom: "22px", color: "#334155", fontSize: "14px", fontWeight: 700 },
  spinner: { width: "28px", height: "28px", border: "3px solid #e2e8f0", borderTop: "3px solid #0a1929", borderRadius: "50%", animation: "spin 1s linear infinite" },
  message: { padding: "13px 16px", borderRadius: "10px", marginBottom: "20px", fontSize: "14px", lineHeight: 1.5 },
  tableContainer: { marginBottom: '24px', border: '1px solid #e2e8f0', borderRadius: '12px', overflow: 'hidden' },
  tableTitle: { padding: '16px 20px', margin: 0, fontSize: '16px', fontWeight: '600', background: '#f8fafc', borderBottom: '1px solid #e2e8f0', color: '#0a1929' },
  ...
  supervisorBadge: { padding: '2px 8px', background: '#dbeafe', color: '#1e40af', borderRadius: '12px', fontSize: '12px', fontWeight: '500' },
  noSupervisorBadge: { padding: '2px 8px', background: '#f1f5f9', color: '#94a3b8', borderRadius: '12px', fontSize: '12px' },
  actionButton: { padding: '4px 12px', background: '#1a237e', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer', fontSize: '12px', fontWeight: '500' },
  emptyState: { textAlign: 'center', padding: '30px', color: '#94a3b8' },
  actions: { display: "flex", justifyContent: "center", gap: "12px", flexWrap: "wrap", marginTop: '16px' },
  primaryButton: { padding: "11px 18px", background: "#0a1929", color: "white", borderRadius: "8px", textDecoration: "none", fontSize: "14px", fontWeight: 800 },
  secondaryButton: { padding: "11px 18px", background: "#f1f5f9", color: "#0a1929", border: "1px solid #cbd5e1", borderRadius: "8px", textDecoration: "none", fontSize: "14px", fontWeight: 800 }
};
